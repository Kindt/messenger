# Spec 028 — Org shell layouts (auth + post-login chrome)

**Feature branch:** `028-org-shell-layouts` (to be created)  
**Created:** 2026-06-26  
**Status:** draft  
**Predecessor:** [`specs/027-ui-branding/`](../027-ui-branding/) — **closed** (visual skin: palettes, tokens, custom CSS, demo skins)
...
Research during 027 closure: **no standalone org portal apps** in repo — layout belongs in web-client shell, configured per org/platform.
...
1. **Pre-login** (`renderAuth`) — structure and responsive behavior.
2. **Post-login** (main app chrome) — nav density and optional sidebar collapse defaults.
...
| Item | Reason |
|------|--------|
| Palettes, token overrides, custom CSS, demo skins | **Spec 027 — closed** |
...
|--------|------|------|--------|
| GET | `/api/v1/branding` | Public | Optional `?org_slug=` → merge org branding + **`shell_layout`** |
| GET | `/api/v1/branding/me` | Bearer | Include resolved **`shell_layout`** (org override → platform default) |
...
- CSS: `shell-layouts.css` — variant modifiers for `.auth-shell`, `.app-shell`, nav/list density.
...
- Admin: dropdown in existing «Персонализация UI» section (027 panel) — **layout only**, no palette duplication.

---

## Testing

- H2: adapter read/write `shell_layout`, merge precedence platform → org.
- Unit: layout enum normalization.
- Playwright: `ui-shell-layouts.spec.ts` — auth-split desktop + mobile stack, compact post-login density smoke.
