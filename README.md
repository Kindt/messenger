...
| **`specs/archive/003-docker-ansible-autotest/`** | Docker + Ansible deploy (archived) — **`deploy/ansible/DEPLOY_QUICKSTART.md`**. |
...
| **`docs/DEV_STACK_PROFILES.md`** | Профили стендов: QEMU **dev/full**, deploy **pilot/standard**, compose **full-server / pilot / dev-min** |
...
| **`docs/parity/`** | Parity web-client: matrix, report, runtime gate (archived spec 002) |
...
| **`docker/docker-compose.full-server.yml`** | Полный стенд без профилей: **dev-min**-инфраструктура + **core-api** + **ws-gateway** (**8082**) + **message-pipeline** + **retention-worker** (**9192** metrics/health) + **export-replay-worker** (**9193** metrics/health) + **push-worker** (**9194** health). Та же сеть **`korus_messenger_dev_min`**. Запуск: **`.\scripts\full-stack-up.ps1`** (**`-Build`**, **`-SkipEnsure`** / **`$env:SKIP_KORUS_ENSURE='1'`**, **`-Help`**, …) или , либо **`./scripts/full-stack-up.sh`** (**`--build`**, **`--skip-ensure`** / **`-S`**, **`SKIP_KORUS_ENSURE=1`**). Остановка: **`.\scripts\full-stack-down.ps1`** (**`-Help`**) / **`./scripts/full-stack-down.sh`** (**`--help`**, **2** попытки **`docker compose down`**). Админка: **`http://localhost:8080/admin/`** (**csadmin**/**csadmin** или **admin**/**admin**, realm **avandocmsg**). Матрица портов: **`docs/PORTS_MATRIX.md`**. |
| **`scripts/TEST_SERVER_READY.md`** | Когда стенд готов к ручным проверкам |
| **`scripts/dev-web-stack-up.ps1`** / **`scripts/dev-web-stack-up.sh`** | Профиль **`web`** в **`docker-compose.dev-min.yml`**; **`KORUS_*`**, встроенная проверка окружения, **2** попытки **`docker compose`**; **`-Help`**; **`-SkipEnsure`** / **`$env:SKIP_KORUS_ENSURE='1'`** / **`SKIP_KORUS_ENSURE=1`** / **`--skip-ensure`** в **`.sh`**; **`-Build` / `--build`**. |
| **`scripts/dev-web-stack-down.ps1`** / **`scripts/dev-web-stack-down.sh`** | Остановка профиля **`web`** (**ws-gateway**, **message-pipeline**): **`docker compose --profile web down`**; **`-Volumes`** / **`--volumes` / `-V`**; **`-Help`**. |
| **`scripts/korus-web-up.ps1`** / **`scripts/korus-web-up.sh`** | Стек **`korus-web/`**; **`KORUS_KORUS_WEB_*`**, проверка окружения, **2** попытки compose; **`-Help`**; **`-Attach` / `--attach`**, **`-Turn` / `--turn` / `-t`** ( **`docker-compose.turn.yml`**, coturn), **`-SkipEnsure`** / **`$env:SKIP_KORUS_ENSURE='1'`** / **`SKIP_KORUS_ENSURE=1`** / **`--skip-ensure`** в **`.sh`**, **`-Build` / `--build`**. |
| **`scripts/korus-web-down.ps1`** / **`scripts/korus-web-down.sh`** | Остановка **`korus-web/`** с теми же **`-f`**, что у **`up`**: **`-Attach` / `--attach`**, **`-Turn` / `--turn` / `-t`**; **`-Volumes`** / **`--volumes` / `-V`** — **`docker compose down -v`**; **`-Help`**. |
| **`scripts/smoke-korus-web.ps1`** / **`scripts/smoke-korus-web.sh`** | Смок **`korus-web`** (health, UI, **`web-client-env.js`** с **`wsUrl`** / **`iceServersJson`**); **`-Help`** / **`--help`**; **`-CheckApi` / `--check-api`** — **`GET …/api/v1/health`** через прокси; в **`.sh`**: **`--url`** / **`WEB_BASE_URL`** |
| **`scripts/dev-ui-hints.ps1`** / **`scripts/dev-ui-hints.sh`** | Вывести URL веб-клиента (порт из **`korus-web/.env`**), админку, Keycloak, тестовые логины realm (**`admin`/`csadmin`**). **`-Help`** / **`--help`** (**`.sh`**: без лишних аргументов). |
