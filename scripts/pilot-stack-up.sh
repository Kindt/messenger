...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  Profiles: push, retention, compliance, archive"
      exit 0
      ;;
    *)
...
if $BUILD && [[ "${KORUS_QEMU_CONSOLE:-0}" = "1" ]]; then
  echo "QEMU: docker compose build pilot (COMPOSE_PARALLEL_LIMIT=${COMPOSE_PARALLEL_LIMIT})" >&2
...
if [[ "$WAIT_READY" == "1" ]]; then
  ...
  if ! $core_ok || ! $kc_ok; then
    echo "[FAIL] pilot stack not ready (core=$core_ok keycloak=$kc_ok)" >&2
    exit 1
  fi
fi

echo "[OK] Pilot stack: core-api :8080, Keycloak :8081 (prod), ws-gateway :8082"
echo "Smoke: ./scripts/smoke-pilot-stack.sh"
