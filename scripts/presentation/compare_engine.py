...
def is_tco_comparable(offering: dict[str, Any]) -> bool:
    return (
        offering.get("tco_comparable", False)
        and offering.get("price_is_public", True)
        and offering.get("metric") == "registered_users"
        and offering.get("billing_unit") == "registered_users"
        and offering.get("product_id") != "korus"
    )


def _yearly_price(offering: dict[str, Any]) -> int | None:
    if not is_tco_comparable(offering):
        return None
...
def build_compare_row(offering: dict[str, Any]) -> CompareRow:
    ...
    headroom = se.headroom_ru(ru) if competitor_yearly is not None else None
    return CompareRow(
        offering=offering,
...
def build_all_rows(offerings: list[dict[str, Any]]) -> list[CompareRow]:
    return [build_compare_row(o) for o in offerings if is_tco_comparable(o)]
