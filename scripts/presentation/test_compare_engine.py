...
def test_headroom_when_profile_allows_more():
    offering = offering_by_id("compass-cloud-390")
    row = build_compare_row(offering)
    assert row.korus_headroom_ru is not None
    assert row.korus_headroom_ru > 100
...
def test_non_public_price_has_no_headroom_badge():
    offering = offering_by_id("loop-enterprise-kp")
    row = build_compare_row(offering)
    assert row.competitor_total_yearly_rub is None
    assert row.korus_headroom_ru is None
    assert row.headroom_note == ""
