...
def test_footer_uses_product_status_build_date(monkeypatch):
    from scripts.presentation import product_status as ps
    from scripts.presentation import render
    ...
    monkeypatch.setattr(render, "date", FakeDate, raising=False)
    html = render.render_deck_html()

    assert f"{ps.PRODUCT_VERSION} | {ps.BUILD_DATE} |" in html
