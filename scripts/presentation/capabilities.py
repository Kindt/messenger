...
CAPABILITY_GROUPS: tuple[tuple[str, str, str], ...] = (
    (
        ...
        "вложения и ответы. Работает в браузере, можно закрепить ярлык «как приложение».",
        "Реализовано",
    ),
    (
        "Состав продукта",
    ...
    (
        ...
        "Появились подключаемые функции для продуктивности и совместной работы: опросы, запланированные сообщения, "
        "напоминания, стикеры/GIF, канбан и доска в чате. Их стоит показывать как сценарии пилота по выбору, "
        "а не как обязательную часть базового контура.",
        "Частично",
    ),
    (
        "AI и распознавание",
    ...
    (
        "Звонки из чата",
        "Аудио- и видеозвонки запускаются прямо из переписки. Для больших групповых созвонов и сложных сетей нужна настройка медиасервера и сетевых правил у заказчика.",
        "Частично",
    ),
    (
        "Live-трансляции",
    ...
    (
        "Администрирование",
        "Веб-консоль для IT и администраторов: организации, пользователи, аудит, политики хранения, импорт переписки, вход через корпоративные системы и управление подключёнными модулями.",
        "Реализовано",
    ),
    (
        "Комплаенс и архив",
