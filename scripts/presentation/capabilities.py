...
CAPABILITY_GROUPS: tuple[tuple[str, str, str], ...] = (
    ...
    (
        "Звонки из чата",
        "Аудио/видео между участниками чата: mesh P2P и групповые конференции через LiveKit SFU. "
        "TURN для NAT/firewall в production настраивает IT заказчика.",
        "Частично",
    ),
    (
        "Live-трансляции",
        "Корпоративные эфиры: RTMP ingress, HLS-плеер, модерация и DVR в admin/API. "
        "Formal soak 10k зрителей — после появления stage (Sep 2026+).",
        "Частично",
    ),
    (
        "Администрирование",
        "Веб-консоль /admin/: организации, пользователи, аудит, ретенция; "
        "мастер auth-policy (LDAP/SSO), directory sync, live-сессии.",
        "Реализовано",
    ),
    (
        "Комплаенс и архив",
    ...
    (
        "Вход и безопасность",
        "Login-options с экрана входа; auth-policy в /admin/ (LDAP, OIDC/SAML, Kerberos scaffold). "
        "SCIM 2.0 Users/Groups для провизионинга. E2EE (MLS) — после sign-off ИБ.",
        "Частично",
    ),
    (
        "Боты и интеграции",
        "Bot API с webhook outbox/retry, платформа плагинов L0–L3, мосты к 1С, почте, хранилищам.",
        "Реализовано",
    ),
    (
        "Мобильные клиенты",
