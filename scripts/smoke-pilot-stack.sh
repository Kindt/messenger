#!/usr/bin/env bash
# Pilot stack acceptance smoke (spec 006 / FR-OPT-01).
set -euo pipefail
...
if docker ps --format '{{.Names}}' 2>/dev/null | grep -qiE 'solr|zookeeper|zoo'; then
  smoke_fail "solr/zookeeper containers must not run on pilot stack"
fi
...
MSG_ID=$(smoke_send_message "$BASE_URL" "$TOKEN_A" "$DM_CHAT" "pilot-dm-$(date +%s)")
pilot_poll_message_id "$BASE_URL" "$TOKEN_B" "$DM_CHAT" "$MSG_ID" 20 || smoke_fail "DM delivery failed"
echo "[OK] DM delivery" >&2

smoke_step "WS deliver"
WS_MARKER="pilot-ws-$(date +%s)"
WS_PID=""
if command -v python3 >/dev/null 2>&1; then
  ...
  if wait "$WS_PID"; then
    ...
    pilot_poll_message_id "$BASE_URL" "$TOKEN_B" "$DM_CHAT" "$WS_MSG" 15 \
...
MARKER="pilot-search-$(date +%s)"
pilot_send_legacy_text "$BASE_URL" "$TOKEN_A" "$SEARCH_CHAT" "$MARKER" >/dev/null
sleep 2
resp=$(smoke_curl_json GET "${BASE_URL%/}/api/v1/search/messages?q=pilot-search&limit=20" "$TOKEN_A") \
...
python3 -c 'import json,sys; data=json.loads(sys.argv[1]); items=data if isinstance(data,list) else (data.get("items") or data.get("results") or []); needle=sys.argv[2]; sys.exit(0 if any(needle in (m.get("content") or "") for m in items) else 1)' "$resp" "$MARKER" \
  || smoke_fail "search did not return pilot message (SQL path)"
echo "[OK] SQL search returned results" >&2

echo ""
echo "[OK] smoke-pilot-stack (spec 006)"
