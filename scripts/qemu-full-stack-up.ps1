...
Write-Host "Refreshing minute status snapshot..." -ForegroundColor DarkGray
& (Join-Path $Root "scripts\qemu-status-minute.ps1") -Once -NoRemediate | Out-Null
& (Join-Path $Root "scripts\start-qemu-status-loop.ps1") -Force | Out-Null
Write-Host "  Minute monitor: deploy\qemu\run\status-minute.log (loop restarted)" -ForegroundColor DarkGray
