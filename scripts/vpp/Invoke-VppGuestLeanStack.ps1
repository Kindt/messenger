...
if ($Help) { Write-Host "Usage: Invoke-VppGuestLeanStack.ps1"; exit 0 }
...
Invoke-PlinkShell -Plink $Plink -HostKey $hk -Port 12221 -Script "cd /mnt/korus && sed -i 's/\r$//' scripts/smoke-lean-stack.sh && bash scripts/smoke-lean-stack.sh"
exit $LASTEXITCODE
