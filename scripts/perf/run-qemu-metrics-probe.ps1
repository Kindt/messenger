...
probe_push() {
  ...
  if [ "$health" = "ok" ]; then
    echo "[OK] $name health (legacy image without /metrics)"
    return 0
  fi
...
if [ -n "$push" ]; then
  push_metrics=$(curl -sf --max-time 8 http://127.0.0.1:9194/metrics || echo "")
  if [ -n "$push_metrics" ]; then
    echo "[OK] push-worker bytes=$(echo -n "$push_metrics" | wc -c)"
  elif curl -sf --max-time 8 http://127.0.0.1:9194/health >/dev/null 2>&1; then
    echo "[OK] push-worker /health ready (metrics not on /metrics yet вЂ” rebuild push-worker image for Prometheus scrape)"
  else
    echo "[FAIL] push-worker unreachable http://127.0.0.1:9194"
    fail=1
  fi
else
