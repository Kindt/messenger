...
# Отладка: debug-baea72.log (NDJSON) + /var/log/korus-bootstrap.log на guest.
...
Logs:
  debug-baea72.log (host, debug session baea72)
...
Write-KorusDebugLog -Location "qemu-dev-clean-up.ps1" -Message "clean dev stand requested" -HypothesisId "ALL" -Data @{
    action = "down then up without KeepDisks"
}
...
Write-KorusDebugLog -Location "qemu-dev-clean-up.ps1" -Message "qemu-up finished; bootstrap runs async on guests" -HypothesisId "ALL"
...
Write-Host "  Get-Content debug-baea72.log -Wait -Tail 20"
