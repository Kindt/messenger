...
for i in $(seq 1 "$BURST"); do
  smoke_send_message "$BASE_URL" "$TOKEN_A" "$CHAT" "load-burst-$i-$(date +%s%N)" >/dev/null
done
END=$(date +%s)
ELAPSED=$((END - START))
...
smoke_poll_message "$BASE_URL" "$TOKEN_B" "$CHAT" "load-burst-$BURST" 30 || smoke_fail "delivery lag after burst"
