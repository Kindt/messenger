...
final class AdminExportEnvChecklist {
    ...
    static List<AdminExportComplianceGuideResponse.EnvChecklistItem> items() {
        return List.of(
            item("EXPORT_REPLAY_INCLUDE_FILE_BODIES", "ZIP with attachment bytes + manifest", DEFAULT_FALSE),
            item("EXPORT_REPLAY_INCLUDE_E2EE_FILE_CANDIDATES", "Heuristic file_metadata for chat members (E2EE chats)", DEFAULT_FALSE),
            item("EXPORT_REPLAY_MAX_E2EE_FILE_CANDIDATES", "Cap for e2eeFileCandidates block", "64"),
            item("EXPORT_ADMIN_SUGGEST_ENABLED", "POST export-compliance-prep, export-suggest", DEFAULT_FALSE),
            item("EXPORT_ADMIN_EXPORT_ENABLED", "POST /admin/chats/{id}/export (queue job)", DEFAULT_FALSE),
            item("EXPORT_REPLAY_INCLUDE_DEEP_ARCHIVE", "Merge deep-archive MinIO message snapshots", DEFAULT_FALSE),
            item("EXPORT_REPLAY_INCLUDE_RETENTION_SNAPSHOTS", "Merge retention hot-body MinIO snapshots", DEFAULT_FALSE),
            item("EXPORT_REPLAY_INCLUDE_SOLR_INDEX", "Attach Solr index dump for chat", DEFAULT_FALSE),
            item("EXPORT_REPLAY_APPLY_MESSAGE_TTL_FILTER", "Exclude messages past per-message TTL", DEFAULT_FALSE),
            item("RETENTION_PUBLISH_EXPORT_SUGGESTED", "Retention worker publishes msg.export.suggested", DEFAULT_FALSE),
            item("EXPORT_AUTO_QUEUE_ON_SUGGESTED", "Auto-queue export on suggestion (deduped)", DEFAULT_FALSE),
            ...
            item("EXPORT_COMPLETENESS_STRICT", "Fail export when mandatory field missing", DEFAULT_FALSE)
        );
    }
