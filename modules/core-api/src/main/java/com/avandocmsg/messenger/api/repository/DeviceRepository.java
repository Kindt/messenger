...
public class DeviceRepository {
    ...
    /**
     * Upsert by (user_id, device_name): обновляет push_token или вставляет новую строку.
     */
    ...
    /** Clears push credentials for the device; returns true if a row was updated. */
    ...
    private java.util.Optional<DeviceResponse> findByUserAndName(UUID userId, String deviceName) {
        var sql = """
            SELECT id, device_name, push_provider, created_at,
              (push_token IS NOT NULL AND btrim(push_token) <> '') AS push_active
        ...
        try (var conn = dataSource.getConnection();
            ...
            try (var rs = stmt.executeQuery()) {
                if (rs.next()) {
                    return java.util.Optional.of(mapDeviceRow(rs));
                }
            }
        } catch (Exception e) {
