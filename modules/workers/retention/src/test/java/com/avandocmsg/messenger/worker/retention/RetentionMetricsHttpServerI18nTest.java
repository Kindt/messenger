...
class RetentionMetricsHttpServerI18nTest {

    @Test
    void healthOkBodyIsLocalizedEnglish() throws Exception {
        var messages = commonMessages(Locale.ENGLISH);
        try (var server = RetentionMetricsHttpServer.start(0, () -> true, messages)) {
    ...
    @Test
    void healthNotReadyBodyIsLocalizedEnglish() throws Exception {
        var messages = commonMessages(Locale.ENGLISH);
        try (var server = RetentionMetricsHttpServer.start(0, () -> false, messages)) {
            var body = getHealthBody(server.getPort());
            assertEquals(503, body.status());
            assertEquals("not ready", body.text().trim());
        }
    }
    ...
    private static UserMessageSource commonMessages(Locale locale) {
        return new CompositeMessageSource(
            locale,
            RetentionMetricsHttpServerI18nTest.class.getClassLoader(),
