...
**Версия документа:** 1.6 (исходник для сборки HTML)  
**Публикация для заказчика:** [`product_presentation.html`](../product_presentation.html) **v2.3** — презентация продукта (не техническое ТЗ).  
**Дата:** 2026-06-15  
...
| Infra optimization (FR-OPT) | Оптимизация §12 | Реализовано (01–07) | spec 006/007; 08–09 и formal load test — roadmap |
...
7. **Инфраструктура (§12):** FR-OPT-01…07 реализованы и приняты на QEMU (spec 006/007); целевые показатели §10.2.1 подтверждаются formal load test на stage при go-live (k6-скрипты готовы).
...
| ID | Требование (что должно быть достигнуто) | Зачем бизнесу | Профиль | Статус |
|----|----------------------------------------|---------------|---------|--------|
| **FR-OPT-01** | Минимальный состав серверов для Pilot (без поискового кластера Solr) | −75% стоимость пилота | Pilot | **Реализовано** |
| **FR-OPT-02** | Аутентификация в production-режиме с предсказуемым потреблением RAM | Стабильный pilot без dev-overhead | Pilot+ | **Реализовано** |
| **FR-OPT-03** | Кэш частых чтений (список чатов, непрочитанные) | Больше пользователей без новых серверов | Standard | **Реализовано** |
| **FR-OPT-04** | Горизонтальное масштабирование доставки сообщений и WebSocket | Рост онлайна и msg/s | Standard+ | **Реализовано** |
...
| Tier | RAM ≤ | Пик msg/s ≥ | Диск (1 год) ≤ | Условие проверки | Статус |
|------|-------|-------------|----------------|------------------|--------|
| 10k | 16 GB | 15 | 6 TB | Load test 20% peak, 24h soak | Инженерная приёмка ✓ (QEMU smokes); load test — при go-live |
| 100k | 160 GB | 120 | 35 TB | idem | Инженерная приёмка ✓ (scale/replica smokes); load test — при go-live |
| 500k | 500 GB | 400 | 120 TB | idem | Запланировано |
| 1M (corp) | 1,2 TB | 600 | 220 TB | idem + sharding plan | Запланировано (FR-OPT-09) |
...
| 2026-06-15 | Spec 006 infra optimization (FR-OPT 01–07); §12 закрыт в презентации |
...
| FR-OPT | Design § | Baseline RAM 100k | Target RAM 100k | Зависит от | Статус |
|--------|----------|-------------------|-----------------|------------|--------|
| 01 Pilot | §1 | 256 GB | — (14 GB @10k) | — | Реализовано |
| 02 Keycloak | §2 | вкл. в baseline | −8 GB | 01 | Реализовано |
| 03 Redis cache | §3 | 256 GB | 180 GB | 01 | Реализовано |
| 04 Scale WS/pipeline | §4 | 256 GB | 160 GB | 01 | Реализовано |
