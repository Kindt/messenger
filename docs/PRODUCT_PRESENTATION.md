...
**Публикация для заказчика:** [`product_presentation.html`](../product_presentation.html) **v2.6.2** — презентация продукта (не техническое ТЗ). 
**Дата:** 2026-06-16 
...
|--------|-----|-------------------|----------------------------------|
| **†** | §10 sizing, DAU, RAM | Аналитическая модель, не замер на железе заказчика | Уточнение таблиц по load test stage |
| **‡** | §10.5 НТ baseline | Тестовый стенд разработки (малый RAM) | p50/p95 и msg/s с stage iron |
| **§** | §10.3, §14 RPO/RTO | Ориентиры для договора, не backup drill | Фиксация в договоре после restore test |
| **¶** | §17 стоимость infra | Ставки VDS на дату презентации | КП по итогам sizing на stage |
| **#** | §4 «Частично» | Код готов; prod — работы IT/sign-off | Статус → «Реализовано» без смены кода |
| **⊕** | §10.7 узел интеграций | Расчёт RAM/vCPU; mock backends | Мониторинг prod, корректировка VM |
...
- Автотесты UI: **34/34** Playwright на тестовом стенде (2026-06-16).
- Профили развёртывания **Pilot** и **Standard** в поставке.
- **Платформа ботов и плагинов (L0–L3):** админка preset/policy/instance, bridges (Exchange, 1С, storage, OCR/AI), polyglot sidecars; отдельный узел интеграций — **реализовано**, проверено на тестовом стенде.
...
- SSO federation, Live-streaming, мобильные клиенты — **не в текущей поставке** (§9); **prod SLA webhook ботов** — ops (§12).
...
| Bot API + платформа плагинов | **Реализовано** — REST L2 + L0–L3, admin, bridges, sidecars |
...
> Единицы и термины: **§3.2**. Колонка «Оговорка» — **§3.3**.

| Домен | Примеры возможностей | Статус | Оговорка |
|-------|---------------------|--------|----------|
...
| Звонки | Видео из чата, комната по ссылке, демонстрация экрана | Частично | Нужен TURN в контуре заказчика — §3.2 (NAT, firewall) |
| E2EE | Legacy + MLS в браузере | Частично | Sign-off ИБ и руководства — §3.2 |
| Push / PWA | Web push, установка как приложение | Частично | Ключи VAPID и проверка на стенде — §3.2 (ops) |
...
| Bot API + платформа плагинов | REST L2; L0–L3 admin, bridges, sidecars | Реализовано | — |
| Live-streaming | RTMP/SRT, HLS, до 10 000 зрителей | Запланировано | Roadmap, не текущая поставка — §3.2 |
| Мобильные клиенты | iOS/Android | Запланировано / вне репо | — |
| Prod HTTPS | Защищённый доступ, секреты не в открытом виде | Частично | Сертификаты, DNS, vault — IT на контуре (§3.2) |
...
- **Действия:** Админ подключает preset (Naumen/Jira/1С) → сотрудник пишет боту → bridge создаёт тикет.
- **Результат:** Интеграция без ручного копирования; код бота не на сервере чатов.
- **Статус:** **Реализовано** — Bot API L2 + платформа плагинов L0–L3; конкретная ITSM — preset/bridge заказчика
...
| §17 | Bot API + платформа плагинов | Реализовано | REST L2; L0–L3 admin, bridges, sidecars; prod webhook SLA — ops |
...
| Направление | Бизнес-ценность | Зависимости |
|-------------|-----------------|-------------|
| Расширение каталога preset bridges | Новые ITSM/ERP без доработки ядра | L2 bridge workers |
...
> **Уже в продукте (не roadmap):** lean Pilot-профиль, Standard с Solr/replica/кэшем, zstd deep-archive, dedup файлов (content-hash), batch Solr, **платформа ботов-плагинов L0–L3**.
...
| Файлы (ГБ / рег. пользов. / год) | 0,5 | 0,4 | 0,3 | 0,25 |

**Расчётные нагрузки (корпоративный профиль):**

| Метрика | 10 000 | 100 000 | 500 000 | 1 000 000 |
|---------|--------|---------|---------|-----------|
| DAU | 5 000 | 40 000 | 150 000 | 250 000 |
| Пик онлайн | 750 | 4 800 | 15 000 | 20 000 |
| Сообщений / день | 200 000 | 1 400 000 | 4 500 000 | 6 250 000 |
| Средний сообщ./с | ~2,3 | ~16 | ~52 | ~72 |
| Пик сообщ./с (×3,5) | ~8 | ~57 | ~182 | ~253 |

**Профиль исходного ТЗ для 1 000 000 рег. пользов.:** 100 000 000 сообщений/день (~1 157 сообщ./с сред., ~4 000 сообщ./с пик) — верхняя граница проектирования для массового consumer-сценария.

**Лабораторный минимум (не prod):** один dev-стек ~6,4 ГБ ОЗУ (server) + ~1,8 ГБ (web).

### 10.2 Профили развёртывания (Pilot / Standard / Enterprise)

| Профиль | Масштаб (рег. пользов.) | ОЗУ | Состав (упрощённо) |
|---------|-------------------------|-----|-------------------|
| **Pilot** | до 10 000 | **12–16 ГБ** | hot DB, redis, nats, minio, keycloak (prod), core-api, ws, pipeline; **без Solr** (SQL search) |
| **Standard** | 10 000–100 000 | **120–160 ГБ** | Pilot + Solr, retention, indexer, read replica, scale hot path |
| **Enterprise** | 100 000–1 000 000 | **450 ГБ–1,2 ТБ** | Full stack, sharding (roadmap), SolrCloud, hot-plug workers |

### 10.3 Рекомендуемые конфигурации

| Параметр | Pilot (10 000) | Standard (100 000) | Enterprise (500 000) | Enterprise (1 000 000) |
|----------|----------------|--------------------|-----------------------|-------------------------|
| **Архитектура** | Lean stack | Кластер | Расширенный кластер | Распределённый |
| **Суммарно ОЗУ** | **~14 ГБ** | **~140 ГБ** | **~450 ГБ** | **~900 ГБ–1,2 ТБ** |
| **Диск (1 год, с файлами)** | **~5 ТБ** | **~30 ТБ** | **~110 ТБ** | **~200 ТБ** |
| **Пик сообщ./с (ориентир)** | **~15** | **~120** | **~400** | **~600** (corp) |
| **Сеть (пик)** | 200 Мбит/с | 1 Гбит/с | 5 Гбит/с | 10 Гбит/с+ |
| **RPO / RTO** | 24 ч / 4 ч | 8 ч / 2 ч | 1 ч / 30 м | 15 м / 15 м |
...
> **Keycloak sizing:** в Pilot prod используется оптимизированный режим Keycloak. Строки «8 ГБ» в детальных таблицах sizing — production HA, не dev/pilot.

> **Справочно:** full-stack monolith на одном хосте (~64 ГБ ОЗУ) — не рекомендуемый профиль для пилота; см. [`product_presentation.html`](../product_presentation.html) §17 (monolith ~44% дороже Pilot).

### 10.4 Примеры сценариев нагрузки

**Сценарий A — «Тихий офис» (10 000 рег. пользов.)** 
...
**Сценарий B — «Распределённая компания» (100 000 рег. пользов.)** 
Утренний пик: массовая рассылка в 50 групп одновременно (~57 сообщ./с пик). Требуется replica БД, 2–3 экземпляра API, отдельный Solr cluster. TURN для региональных офисов.

**Сценарий C — «Федеральный масштаб» (1 000 000 рег. пользов., профиль ТЗ-max)** 
100M сообщений/день, пик ~4 000 сообщ./с. Распределённый кластер, sharded PostgreSQL, SolrCloud 9+ nodes, geo-реплика MinIO. Отдельная команда SRE 24/7. **Обязателен** load test перед sign-off.
...
| Message pipeline burst (guest) | **50** msg за **8 s**, **~6 сообщ./с** e2e | NATS + pipeline + доставка |
| Messaging E2E + load | **PASS**, 30 load rounds | DM, group, WS/REST, read receipts |

**Сравнение с целью Pilot/S-10k:** проектный пик **~15 сообщ./с**; на dev VM замер e2e burst **~6 сообщ./с** (~40% цели) — ожидаемо для лабораторного железа (~6–10 ГБ guest). Formal soak 20% peak — на stage/prod iron.

### 10.6 Дисклеймер

- Лабораторный baseline (§10.5) — **не** заменяет formal soak на stage; только инженерная проверка (**‡** — см. легенду сносок).
- Активные видеозвонки и live-стримы могут **удвоить** сетевые и CPU требования.
- Рекомендация: validation phase (k6/Locust) на 10–20% целевой нагрузки перед prod — после ops sign-off цифры §10 обновляются.

### 10.7 Узел ботов и плагинов (1 экземпляр = 1 бот)

Исполнение ботов и плагинов вынесено на **отдельный узел интеграций** (не на сервер чатов). Код плагинов **не** загружается в JVM core-api — только HTTP-маршрутизация и org-политики. Каждый зарегистрированный бот = **один экземпляр** (L0…L3).

| Класс | Примеры | RAM / экз. | vCPU / экз. |
|-------|---------|------------|-------------|
| **L0** | FAQ, телефонный справочник, ссылки | 0 (конфиг) | 0 |
| **L1** sidecar | PHP/Go/Python echo, Bitrix sidecar | ~192 МБ | 0,1 |
| **L1** connector | Jira/Confluence read-only mapping | shared runtime | 0,05 |
| **L2** bridge | Exchange, 1С, Naumen, storage, outbound | ~384 МБ | 0,25 |
| **L3** + OCR | AI triage + on-prem OCR | ~768 МБ | 0,5 |

**База узла:** gateway + connector-runtime + ОС ≈ **1,2 ГБ** RAM. Рекомендуемая RAM VM = расчёт + 25% запас (минимум 4 ГБ для pilot).

| Профиль заказчика | Состав (экз.) | Ботов | RAM расч. | RAM VM | vCPU |
|-------------------|---------------|-------|-----------|--------|------|
| Pilot — FAQ + 2 интеграции | L0 FAQ, sidecar PHP, bridge 1С | 3 | ~1 968 МБ | **4 ГБ** | 1,4 |
| Standard — Service Desk | Naumen L2, Jira L1, storage L2, echo sidecar | 4 | ~2 352 МБ | **4 ГБ** | 1,7 |
| Standard — документооборот | OCR+AI L3, exchange L2, 2× FAQ L0 | 4 | ~2 544 МБ | **4 ГБ** | 1,8 |
| Расширенный витрина | 6× L2 bridge + 2 sidecar + 1 L3 | 9 | ~5 088 МБ | **8 ГБ** | 3,5 |

**Сравнение с eXpress @1k рег.:** кластер Bot ~6 vCPU / 12 ГБ RAM (shared). У Korus при 6 L2-bridge: ~**5 ГБ** на узле интеграций — линейно по числу экземпляров, без per-user лицензии на бота.
...
2. **Автоматические UI-сценарии:** **34/34** Playwright на тестовом стенде (2026-06-16).
...
11. **Интеграции (§12):** REST API, WebSocket, Bot API L2 и **платформа плагинов L0–L3** реализованы; SSO — в roadmap.

---

## 12. Интеграции и экосистема

> **Статус раздела:** **реализовано** (REST, WebSocket, export, Bot API L2, **платформа плагинов L0–L3**); **частично** — SSO federation, prod SLA webhook; SSO federation — **запланировано**.
> Связь: КУ-25, КУ-27 (§5); sizing ботов — §10.7.
...
| **Bot API (REST L2)** | IT, интеграторы | register, webhook, long-poll, sendMessage, pin/ban, rotate | Реализовано (prod webhook SLA — ops) |
| **Платформа плагинов L0–L3** | IT, интеграторы | Admin preset/policy/instance; bridges; polyglot sidecars | **Реализовано** |
...
|---------|------------------------|--------|
| **Бот как участник чата** | @имя_бота в группе; ответы в переписке | Реализовано |
| **Права бота** | Только по @mention или чтение всех сообщений (решение модератора) | Реализовано (MENTIONS_ONLY / READ_ALL) |
| **Доставка событий** | Webhook или long-poll; повторы допустимы — бот дедуплицирует | Реализовано (prod SLA webhook — ops) |
| **Методы бота** | Отправка/удаление сообщений, pin, ban/mute в группе | Реализовано |
| **Self-host бота** | Код бота **не** на сервере мессенджера | Реализовано |
| **Аудит** | Создание бота, webhook URL, export событий бота | Реализовано |
...
**Требование FR-INT-06:** типовые сценарии (Service Desk, опрос, FAQ) описаны в **руководстве интегратора**.

### 12.5 Платформа ботов и плагинов (L0–L3)

> **Ключевое отличие:** гибкая система интеграций без загрузки кода в ядро мессенджера. Каждый бот — **отдельный экземпляр** на узле интеграций (§10.7).

| Уровень | Что это | Примеры preset | Статус |
|---------|---------|----------------|--------|
| **L0** | Конфигурация (кнопки, FAQ, ссылки) | Телефонный справочник, меню | Реализовано |
| **L1** sidecar | Polyglot HTTP-сервис (PHP, Go, Python, Java 8, VB.NET, PowerShell) | Echo, Bitrix sidecar | Реализовано |
| **L1** connector | REST mapping в connector-runtime | Jira read-only | Реализовано |
| **L2** bridge | Фоновый worker + внешняя система | Exchange, 1С, Naumen, storage, outbound | Реализовано |
| **L3** | AI + on-prem OCR | AI triage, OCR invoice demo | Реализовано (demo) |

**Админка (3 уровня):** preset каталог → org policy → instance с секретами. Интегратор подключает bridge без пересборки core-api.

**Требование FR-INT-09:** сбой узла интеграций **не блокирует** переписку людей — только боты и bridges.

**Требование FR-INT-10:** sizing ботов — **линейно** по числу экземпляров (§10.7), не per-user.

### 12.6 Интеграция с системами заказчика (паттерны)

| Паттерн | Пример | Как сегодня / план |
|---------|--------|-------------------|
| **Pull export** | SIEM забирает архив чата | Export JSON/ZIP + audit | Реализовано |
| **Push webhook (бот)** | Jira создаёт тикет из сообщения | Bot API + L2 bridge preset | Реализовано |
...
### 12.7 Ограничения и безопасность интеграций
...
### 12.8 Связь с профилями §15

| Возможность | Pilot | Standard | Enterprise |
|-------------|:-----:|:--------:|:----------:|
| REST + WS для custom apps | ✓ | ✓ | ✓ |
| SSO OIDC/SAML | опционально | ✓ | ✓ multi-IdP |
| Bot API + платформа L0–L3 | ✓ | ✓ | ✓ + SLA webhook |
| Узел интеграций (отдельная VM) | опционально | ✓ | ✓ HA |
...
| 2026-06-16 | Playwright 34/34; mesh audio-first; security gate; plugin platform L0–L3 |
| 2026-06-16 | Презентация v2.6.1: оговорки «Частично», единые единицы (ГБ, сообщ./с) |
